...
CONTAINS
  ...
  SUBROUTINE emdep_xtsurf_vdiff
    ...
    INTEGER,  PARAMETER :: nobs_max  = 30               ! maximum number of observed parameters
    ...
    REAL(dp) :: fAPIN, fBPIN, fSQTERP                   ! ESS_lg_20130119+ added the partitioning of monoterpene and sqterp emissions
	REAL(dp) :: vpair, vpleaf, vpsfc, rbveg             ! ESS_lg_20130424+

    INTEGER  :: ntrac
    REAL(dp) :: recalcmrconc                            ! term to recalculate mixing ratio to conc.
    ...
    ntrac=17                             ! ESS_lg_20130503+ added CO2
    IF (l_xtsurf_veg_mlay_chem) ntrac=63 ! ESS_lg_20130503+ added CO2
    ...
    ! MAQ_lg_20160621+ 
    ! Canopy structure parameters for the Daintree analysis on changing phenology
	laibase(:)        = zlai	 
    IF (casename.EQ.'Daintree'.OR.casename.EQ.'Daintree, 1-year') THEN
      DO i=1,Nstep
        laibase(:)=3.5  ! MAQ_lg_20160817+ assuming that the long-term average LAI is 3.5 m2 m-2 (default zlai)
  	    lai(i) = zlai+(1./3.*zlai*COS((2.*PI*i)/(31536000./dtimestep)))  ! zLAI, changed to contain scaling by D. Zweekhorst 1-12-2015, 31536000 = total amount of seconds in a year
      ENDDO
	ENDIF
    ! MAQ_lg_20160621-  	
	...
	ELSEIF (iladprof == 2) THEN 
	  ...
	  ELSE IF (nveglay_hr == 6) THEN    ! with ~75% in crownlayer 
        lad(:,1)        = 0.2           ! LAD first crown-layer
        lad(:,2)        = 0.35          ! ...
        lad(:,3)        = 0.15          ! ...
        lad(:,4)        = 0.1           ! ...
        lad(:,5)        = 0.1           ! ...
        lad(:,6)        = 0.1           ! LAD lowest understory-layer
      ElSE
        write(*,'(1a,i3,1a)') &
   ...
   &                    COS ((15./360.)*(timeday(i)-12)*2.*PI))  
       ...
       tamin=tmin+0.25*(tmax-tmin)
       tamax=tmax-0.25*(tmax-tmin)
       ! temporal cycle in Tsoil with a dampened temporal cycle compared to Tsurf
       tslmin=tmin+0.25*(tmax-tmin)
       tslmax=tmax-0.25*(tmax-tmin)
       ...
       um1(i)    =   umin+(umax-umin)*scaling
       vm1(i)    =   vmin+(vmax-vmin)*scaling
    ...
    IF (l_emis_bio_VOC) THEN
        ...
        fSQTERP             = zfSQTERP    ! fraction of monoterpene emission emitted as a-pinene
    ENDIF
    ...
    IF (l_readdata) THEN
       ...
       DO i=1,Nstep
         ...
         ! ESS_20130402+ other specific cases for which MLC-CHEM has been applied

         ! CLAIRE campaigns, IOP2, October/November 2012
         IF (casename.EQ.'CLAIRE2012-2014, IOP2, Oct/Nov 2012') THEN

		   !pxtm1_obs(i,idt_NO)=1e-12*10.  ! only assigning NO when NO > 0
           !pxtm1_obs(i,idt_NO2)=1e-12*60. ! only assigning NO2 when NO2 > 0

           ! only assigning a value when not NA
           IF (observ(i,14) > -9999.) pxtm1_obs(i,idt_CO2)=observ(i,14)*1e-6 ! observed CO2 is given in ppmv, only assigned when not NA
           IF (observ(i,15) > -9999.) pxtm1_obs(i,idt_O3)=observ(i,15)*1e-9  ! observed O3 is given in ppbv, only assigned when not NA
           
		   pxtm1_obs(i,idt_CO)=1e-9*80.  ! observed CO is given in ppbv, only assigned when not NA

           IF (observ(i,5)> -9999.) THEN
		     globrad(i)=MAX(0.,observ(i,5))
    	     IF (observ(i,6) > -9999.) netrad(i)=MAX(0.,observ(i,5)-observ(i,6))   ! net radiation 

             ! -------------------------------------------------------------------------------
             ! ESS_lg_20130228+ whenever the observed net radiation is being used then the
			 ! non-observed radiation dependent parameters should be corrected for the difference 
			 ! between the maximum (cloud-free) net radiation and the actual net radiation. 
			 ! First of all here the maximum net radiation for the site is calculated. 
			 ! In a follow-up timeloop below, this netradmax, scaled with the cos zenith angle 
			 ! is then used to calculate this radiation scaling function
             ! -------------------------------------------------------------------------------

			 IF (netrad(i) > netradmax) netradmax=netrad(i)

		   ENDIF
           ! ESS_lg_20130228- 

           ! observed air temperature at 73m (temperature measured in atmospheric surface layer) 
		   ! it seems that the T measured at 81m had some issues based on analysis of the IOP2 data
           IF (observ(i,10) > -9999.) tair(i)=observ(i,10)+273.15 ! assigning the read-in air temperature [K]
                 
           ! observed surface/skin/canopy temperature (temperature measured at the top of the canopy) 
           IF (observ(i,8) > -9999.) tsurf(i)=observ(i,8)+273.15 ! assigning the read-in surface temperature [K]
                 
           ! observed soil temperature (temperature measured in soil, e.g., in the top few cm of the soil) 
           IF (observ(i,7) > -9999.) tsoil(i)=observ(i,7)+273.15 ! assigning the read-in soil temperature [K]
           ...
           ! not having the individual U (x- or West-East direction) and V (y- or North-South direction component) then
           ! assign the observed windspeed to U and set vm1 to 0!           
           IF (observ(i,13) > -9999.) um1(i)=observ(i,13) ! assigning the read-in windspeed, U-component [m s-1]

           ! observed windspeed (V-component, measured in atmospheric surface layer) 
           IF (observ(i,13) > -9999.) vm1(i)=0. ! assigning the read-in windspeed, V-component [m s-1]

          ! observed relative humidity
           IF (observ(i,3) > -9999.) rh(i)=observ(i,3)/100. ! assigning the read-in relative humidity, expressed as value 0-1

           ! observed precipitation
           IF (observ(i,4) > -9999.) prc(i)=1e-3*observ(i,4) ! assigning the read-in precipitation from the example file
                                                             ! observ(:,6) is in mm whereas prc is in 
           prectot(i)=0.3 ! ESS_lg_20150204+ added the monthly accumulated precipitation in m to secure use of
		                  ! a wet or dry season soil NO emission factor, the threshold is at 0.15m 
															 															 
		 ENDIF ! END IF (casename='CLAIRE2012-2014, IOP2, Oct/Nov 2012') THEN

        ! CLAIRE campaigns, IOP2, October/November 2012
         IF (casename.EQ.'CLAIRE2012-2014, March 2014'.OR.casename.EQ.'CLAIRE/ATTO 052013-122014'.OR. &
		     casename.EQ.'CLAIRE/ATTO 11062013-13062013'.OR.casename.EQ.'CLAIRE/ATTO 21092013-26092013'.OR. &
			 casename.EQ.'CLAIRE/ATTO 18102015-20102015') THEN

		   !pxtm1_obs(i,idt_NO)=1e-12*10.  ! only assigning NO when NO > 0
           !pxtm1_obs(i,idt_NO2)=1e-12*60. ! only assigning NO2 when NO2 > 0

		   IF (casename.EQ.'CLAIRE/ATTO 18102015-20102015') THEN
		      ! only assigning a value when not NA
              IF (observ(i,19) > -9999.) pxtm1_obs(i,idt_CO2)=observ(i,19)*1e-6 ! observed CO2 is given in ppmv, only assigned when not NA
              IF (observ(i,18) > -9999.) pxtm1_obs(i,idt_O3)=observ(i,18)*1e-9  ! observed O3 is given in ppbv, only assigned when not NA
           ELSE
              ! only assigning a value when not NA
              IF (observ(i,17) > -9999.) pxtm1_obs(i,idt_CO2)=observ(i,17)*1e-6 ! observed CO2 is given in ppmv, only assigned when not NA
              IF (observ(i,16) > -9999.) pxtm1_obs(i,idt_O3)=observ(i,16)*1e-9  ! observed O3 is given in ppbv, only assigned when not NA
           ENDIF
           ...
           ! MAQ_lg_20160923+ assuming a strongly reduced nocturnal wind speed to mimic a strong inversion for the dataset 
           IF (globrad(i)<1.AND.observ(i,13) > -9999.) um1(i)=0.25*observ(i,13)
           ...
           IF (observ(i,4) > -9999.) prc(i)=1e-3*observ(i,4) ! assigning the read-in precipitation from the example file
                                                             ! observ(:,6) is in mm whereas prc is in 
           prectot(i)=0.120 ! ESS_lg_20150204+ added the monthly accumulated precipitation in m to secure use of
		                  ! a wet or dry season soil NO emission factor, the threshold is at 0.15m 
															 															 
		 ENDIF ! END IF (casename='CLAIRE2012-2014, March 2014') THEN		 
		 		 
		 ! only assigned when not NA but also >= 0 (not using small negative concentrations)
         IF (casename.EQ.'Daintree'.OR.casename.EQ.'Daintree, 1-year') THEN

		   IF (casename.EQ.'Daintree, 1-year') THEN
		   
 	 	     IF (observ(i,1) > 0.) tair(i)=observ(i,1)+273.15  
             IF (observ(i,1) > 0.) tsurf(i)=observ(i,1)+273.15 ! only assigning NO2 when NO2 > 0

             ! observed precipitation
             IF (observ(i,3) > -9999.) prc(i)=1e-3*observ(i,3) ! assigning the read-in precipitation from the example file
                                                             ! observ(:,6) is in mm whereas prc is in 
             IF (observ(i,4) > -9999.) netrad(i) =MAX(0.,observ(i,4))   ! net radiation 
             IF (netrad(i) > netradmax) netradmax=netrad(i)

             ! to mimic stability effects; the term 0.0022 is based on a maximum temperature increase of 2K 
		     ! for a maximum net radiation of 900 W m-2
		     IF (observ(i,1) > 0. .AND. netrad(i).gt.0.) THEN
		       tsurf(i)=observ(i,1)+273.15+netrad(i)*0.0022 ! only assigning NO2 when NO2 > 0		   
             ELSEIF (observ(i,1) > 0. .AND. netrad(i).lt.1e-5) THEN
               tsurf(i)=observ(i,1)+273.15-1. ! nighttime stable conditions
		     ENDIF
 
		     ! observed windspeed (U-component, measured in atmospheric surface layer); when only the windspeed is known
             ! not having the individual U (x- or West-East direction) and V (y- or North-South direction component) then
             ! assign the observed windspeed to U and set vm1 to 0!           
             IF (observ(i,5) > -9999.) um1(i)=observ(i,5) ! assigning the read-in windspeed, U-component [m s-1]
		     IF (observ(i,6) > -9999.) vm1(i)=observ(i,6) ! assigning the read-in windspeed, V-component [m s-1]
		     IF (observ(i,7) > -9999.) THEN
			   zcvw(i)=observ(i,7) ! assigning wet skin fraction/leaf wetness
               l_wetskinRH=.false. ! to avoid overwriting read-in wet skin fraction with this fraction calc. from RH
             ENDIF			   

             ! only assigning a value when not NA
             IF (observ(i,8) > -9999.) pxtm1_obs(i,idt_O3)=1e-9*observ(i,8)  ! observed O3 is given in ppbv, only assigned when not NA
		     IF (observ(i,9) > 0.)     pxtm1_obs(i,idt_NO)=1e-9*observ(i,9)  ! only assigning NO when NO > 0
             IF (observ(i,10) > 0.)    pxtm1_obs(i,idt_NO2)=1e-9*observ(i,10) ! only assigning NO2 when NO2 > 0
			 
		   ELSEIF (casename.EQ.'Daintree') THEN
             ...
             ! not having the individual U (x- or West-East direction) and V (y- or North-South direction component) then
             ! assign the observed windspeed to U and set vm1 to 0!           
             IF (observ(i,8) > -9999.) um1(i)=observ(i,8) ! assigning the read-in windspeed, U-component [m s-1]
		     IF (observ(i,9) > -9999.) vm1(i)=observ(i,9) ! assigning the read-in windspeed, V-component [m s-1]

		     IF (observ(i,11) > 0.) pxtm1_obs(i,idt_NO)=1e-9*observ(i,11)  ! only assigning NO when NO > 0
             IF (observ(i,12) > 0.) pxtm1_obs(i,idt_NO2)=1e-9*observ(i,12) ! only assigning NO2 when NO2 > 0
             ! only assigning a value when not NA
             IF (observ(i,10) > -9999.) pxtm1_obs(i,idt_O3)=1e-9*observ(i,10)  ! observed O3 is given in ppbv, only assigned when not NA
    ...
    IF (l_emis_bio_VOC.OR.l_xtsurf_veg_mlay_chem.OR.l_xtsurf_AGS) THEN
        ...
        CALL emdep_xtsurf_calcprof( Nstep, & ! ESS_lg_20120722+
           nveglay_hr, netrad, cossza, laibase, lad,   &
           rbvd, rvdsl, rvd, fslbase)  ! mz_lg_20050721+ added SL diffusive rad.
        ! MAQ_lg_20160621-
		   
        CALL emdep_xtsurf_calcprof( Nstep, & ! ESS_lg_20120722+
           nveglay_hr, netrad, cossza, lai, lad,   &
        ...
        WRITE(2,'(1a)') &
           'LAD profile, net. radiation, diffusive beam comp., frac. sunlit leaves (FSL) and scaled with LAI'
		   
		! to assign parname and parunit for the header of the file
		ALLOCATE (parname(2,nveglay_hr))
		ALLOCATE (parunit(2,nveglay_hr))
		...
		WRITE(2,'(A10,A16,50A15)') &
           'istep','ldatltime',(parname(1,i),i=1,nveglay_hr),'Net. rad.', &
		                       (parname(2,i),i=1,nveglay_hr),'FSL-sum','FSL*LAI'
        WRITE(2,'(A10,A16,50A15)') &
           '#','dd-mm-yy hh:min',(parunit(1,i),i=1,nveglay_hr),'W m-2', &
		                         (parunit(2,i),i=1,nveglay_hr),'[-]','[m2 m-2]'
        DO i=1,Nstep
          IF (mod(i,nprint) == 0) THEN
            WRITE(2,'(I10,2x,A14,50(1x,E14.6))') &
                i,ldatltime(i),(LAD(i,ii),ii=1,nveglay_hr),netrad(i), &
				               (rvd(i,ii),ii=1,nveglay_hr),fslsum(i),fslsum(i)*lai(i)
          ENDIF
        END DO
    ...
    IF (casename.EQ.'Daintree'.OR.casename.EQ.'Daintree, 1-year') THEN
      print *,'resistances for CH2O: rmes ',rmes(idt_CH2O),' rws: ',rws(idt_CH2O),'rcut: ',rcut(idt_CH2O)
	  print *,'ENTER to continue'
      read (*,*)
    ENDIF
    ...
    IF (l_emis_bio_jNO3) THEN
      ...
      ddNO3(:)=pxtm1(:,idt_HNO3)*(1e6*1.e-3_dp*prhoa(:)/(amd/avo))*1./(zrahveg(:))

      CALL emdep_emis_jNO3( Nstep,                                           & ! ESS_lg_20120722+
	  ...
	  IF (l_xtsurf_veg_mlay) THEN
        ...
        DO i=1,Nstep
          IF (mod(i,nprint) == 0) THEN
            WRITE(2,'(I10,2x,A14,50(1x,E14.6))') &
              i,ldatltime(i),ddNO3(i),NO3s(i),(prc(i)+prl(i))*1e3,cthru(i),fslsum(i),  &
             (HONO_emflux(i,ii),ii=1,nveglay),(NOx_emflux(i,ii),ii=1,nveglay)
          ENDIF
        END DO
      ELSE  ! "big-leaf" HONO and NOx emissions from NO3 photolysis
        WRITE(2,'(A10,4A15)') &
          'istep','ddNO3','NO3s','HONO_emflux','NOx_emflux'
        WRITE(2,'(A10,4A15)') &
          '#','molec m-2 s-1','molec m-2','molec m-2 s-1','molec. m-2 s-1'
        DO i=1,Nstep
           IF (mod(i,nprint) == 0) THEN
             WRITE(2,'(I10,2x,A14,8(1x,E14.6))') &
               i,ldatltime(i),ddNO3(i),NO3s(i),cthru(i),fslsum(i),HONO_jNO3em(i),NOx_jNO3em(i)
           ENDIF
        END DO
      ENDIF
	...
	IF (l_xtsurf_AGS) THEN
      ...
      WRITE(2,'(A10,A16,50A15)') &
         'istep','ldatltime','/rco_AGS_bigl',(parname(1,i),i=1,nveglay),'rmesCO2'
      WRITE(2,'(A10,A16,50A15)') &
         '#','dd-mm-yy hh:min','[cm s-1]',(parunit(1,i),i=1,nveglay),'[s m-1]'
      DO i=1,Nstep
        IF (mod(i,nprint) == 0) THEN
          WRITE(2,'(I10,2x,A14,50(1x,E14.6))') &
              i,ldatltime(i),100./rco_leaf_AGS(i),(100./rco_leaf_AGS_ml(i,ii),ii=1,nveglay),rmesCO2(i)
        ENDIF
      END DO
    ...
    IF (l_xtsurf_veg_mlay) THEN
       ...
       CALL emdep_xtsurf_veg_mlay( Nstep,                              & ! ESS_lg_20120722+
            ...
            l_drydep, loland, zfrl, zcvbs, zcvw, zvgrat, zrefsl,       & ! ESS_lg_20131125+ zref, reference height surface layer
			zrahveg, zustveg, u_sl, u_veg,                             & ! ESS_lg_20131125+ u_sl 
			netrad, rj, rj_veg, tsurf, press, qm1, rh,                 & ! ESS_lg_20120725+
            ...
            weight_pxtm1_obs, Kh, patmbiosflux, pcrf, pvdveg, psurfflux, & ! ESS_lg_20120903+ weight_pxtm1_obs ! ESS_lg_20120722+ added Kh for diagnostics
	   ...
	   IF (l_xtsurf_veg_mlay_chem) THEN
         WRITE(2,'(A10,A16,350A15)') &
              ...
              ,'LAI','stom. cond-cl.','ws','f-ws','scale_rad','f-wetskin'                 & ! MAQ_2060621+ added LAI ! ESS_lg_20150406+ f-wetskin ! ESS_lg_20150324+ add scaling_rad ! ESS_lg_20130127+ added soil moisture
...
!              ,'VdAPINP1a_cl','VdAPINP1a_sll'                                             &
              ...
              ,'atmbioflx-CO2','atmbioflx-Rad'                                            & ! ESS_lg_20150611+ Radon ! ESS_lg_20130619+
		 ...
		 WRITE(2,'(A10,A16,350A15)') &
              ...
              ,'m2 m-2','cm s-1','m','0-1','0-1','0-1'                                    & ! MAQ_20160621+ added LAI ! ESS_lg_20150406+ f-wetskin ! ESS_lg_20150324+ added scaling_rad ! ESS_lg_20130127+ added soil moisture
           ...
           IF(mod(i,nprint) == 0) THEN
             ...
             WRITE(2,'(I10,2x,A14,350(1x,E14.6))')                                         &
				...
				,Kh(i,1:nveglay)                                                           & 
                ,lai(i),100./rco_leaf(i,1),ws(i),fws(i),scaling_rad(i),zcvw(i)             & ! MAQ_lg_20160621+ added LAI ! ESS_lg_20150324+ added scaling_rad ! ESS_lg_20130127+ added soil moisture
				,rj(i,idt_NO2),rj_veg(i,1:nveglay,idt_NO2)                                 & 
                ,mixratios(i,1:nveglay+1,1:jjt)                                            &
                ,pvdveg(i,1:nveglay,idt_O3)                                                &
				,pvdveg(i,1,idt_NO2),pvdveg(i,nveglay,idt_ISOP),pvdveg(i,1,idt_CH2O)       & ! ESS_lg_20150406+ VdCH2O
				,pvdveg(i,1,idt_CO2),pvdveg(i,nveglay,idt_CO2)                             & ! ESS_lg_20130811+ VdCO2
                ,pvdveg(i,1,idt_apin),pvdveg(i,nveglay,idt_apin)                           &    

!                ,pvdveg(i,1,idt_APINP1a),pvdveg(i,nveglay,idt_APINP1a)                     &

                ,NO_slflux(i),NOx_emflux(i,1),NOx_emflux(i,nveglay)                        &
                ...
                ,radon_slflux(i)                                                           & ! ESS_lg_20150611+ radon soil flux				
                ,patmbiosflux(i,idt_O3),patmbiosflux(i,idt_NO),patmbiosflux(i,idt_NO2)     &
                ,patmbiosflux(i,idt_HONO)                                                  & ! ESS_lg_20150823+ added HONO
                ,patmbiosflux(i,idt_ISOP),patmbiosflux(i,idt_APIN),patmbiosflux(i,idt_SQTERP) &                      
                ,patmbiosflux(i,idt_CO2),patmbiosflux(i,idt_RAD)                           & ! ESS_lg_20150611+ Radon flux ! ESS_lg_20130619+ added CO2
                ...
                ,-1e-4*patmbiosflux(i,idt_O3)/(recalcmrconc*pxtm1(i,idt_O3))                 ! ESS_lg_20150624+ Canopy-scale VdO3
			 ENDIF
		 END DO
	   ...
	   DO jt=1,ntrac
     	 IF (latmbios_output(jt)) THEN
           IF (jt == idt_NO2 .or. jt == idt_ISOP .or. jt == idt_CO2) THEN
             jjt_emis=jjt_emis+1
		     ! Only canopy layers
		     DO jk=1,nveglay
			   nlevels(npar)=nlevels(npar)+1
	   ...
	   DO jt=1,ntrac
 	  	 IF (latmbios_output(jt)) THEN
           IF (jt == idt_O3 .or. jt == idt_HNO3 .or. jt == idt_NO2 .or. jt == idt_CO2 .or. jt == idt_CH2O) THEN
             jjt_dryd=jjt_dryd+1
		     ! Only canopy layers
		     DO jk=1,nveglay
			   nlevels(npar)=nlevels(npar)+1
	   ...
	   DO jt=1,ntrac
 	  	 IF (latmbios_output(jt)) THEN
           IF (jt == idt_O3 .or. jt == idt_ISOP .or. jt == idt_NO2 .or. jt == idt_CH2O) THEN
 		     nlevels(npar)=nlevels(npar)+1
		     jjt_chem=jjt_chem+1
	         dummy=trname(jt)
             ...
             iip=INDEX(dummy,' ')
 		     dummy3='m)'
             parname(npar,nlevels(npar))=dummy(1:iip-1)//dummy3
             parunit(npar,nlevels(npar))='ppbv s-1'
	   ...
	   DO jt=1,ntrac
 	  	 IF (latmbios_output(jt)) THEN
           IF (jt == idt_O3 .or. jt == idt_ISOP .or. jt == idt_NO2 .or. jt == idt_CO2 .or. jt == idt_CH2O) THEN
 		     nlevels(npar)=nlevels(npar)+1
		     jjt_diff=jjt_diff+1
	         dummy=trname(jt)
             ...
             parname(npar,nlevels(npar))=dummy(1:iip-1)//dummy3
             parunit(npar,nlevels(npar))='ppbv s-1' 
             IF (jt == idt_CO2) parunit(npar,nlevels(npar))='ppmv s-1'
    ...
    ! ESS_lg_20150714+ extra diagnostics on formaldehyde column for Daintree project
    IF (casename.EQ.'Daintree'.OR.casename.EQ.'Daintree, 1-year') THEN
      print *,'Calculation and writing of HCHO column for comparison with RS data'
      ! ESS_20150714+ writing of formaldehyde column for evaluation with RS data
      OPEN(unit=2,file='output/HCHO_VCD.out',status='UNKNOWN')
      WRITE(2,'(1a)') &
         'HCHO VCD based on the model calculated concentration of HCHO in mixed layer'
      WRITE(2,'(A10,A16,50A15)') &
          'istep','ldatltime','HCHO_VCD'
      WRITE(2,'(A10,A16,50A15)') &
          '#','dd-mm-yy hh:min','1e16 molec cm-2'
      DO i=1,Nstep
        ! recalculation factor to arrive at 1e16 molecules HCHO cm-2; prhoa is in kg m-3
		! to go from kg m-3 to g cm-3, multiplying with 1e3/1e6 = 1e-3, then multiplying with the
		! mixed layer depth, expressed in cm gives the recalculation factor to be multiplied with the
		! HCHO mixing ratio
	    recalc_conc_VCD=1e-16*(1.e-3_dp*prhoa(i)/(amd/avo))*MLH(i)*1e2
	    IF (mod(i,nprint) == 0) THEN
          WRITE(2,'(I10,2x,A14,50(1x,E14.6))') &
            i,ldatltime(i),pxtm1(i,idt_CH2O)*recalc_conc_VCD
        ENDIF
	  ENDDO
	ENDIF
    ! ESS_lg_20150714-
  ...
  SUBROUTINE emdep_xtsurf_tracer_init( l_xtsurf_veg_mlay_chem,           &
    ...
    ! the next 14 tracers are included in the non-chemistry as well as the CBM4 tracer list
    ...
    DO i=1,ntrac
       ...
       IF (trname(i) == 'NO      ') pxtm1(:,i)=0.2e-9_dp
       IF (trname(i) == 'NO2     ') pxtm1(:,i)=0.2e-9_dp
    ...
    DO i=1,ntrac
	   ...
	   IF (trname(i).eq.'CH4'.OR.trname(i).eq.'CO'.OR.    &
	       trname(i).eq.'SO2'.OR.trname(i).eq.'NH3') latmbios_output(i)=.false.
