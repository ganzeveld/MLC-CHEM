MODULE messy_emdep_emis
  ...
  USE messy_main_constants_mem,   ONLY: pi, N_A, M_air, g, dp, i4
  ! mz_lg_20040429- 
  ...
  LOGICAL, PUBLIC :: l_emis_soil_HONO_dummy = .false.  ! global switch (namelist) ! MAQ_lg_20170517+
  ! mz_lg_20040921-
  ...
  LOGICAL, PUBLIC :: l_emis_soil_HONO   = .false.! global switch (namelist) ! MAQ_lg_20170517+
  ...
  REAL, PUBLIC    :: zco2emis           =  7.e-6 ! CO2 emission flux [mol CO2 m-2 s-1]
  REAL, PUBLIC    :: znh3emis           = 1.e-10 ! NH3 emission flux [mol NH3 m-2 s-1]
...
CONTAINS
  ...
  SUBROUTINE emdep_emis_read_nml_ctrl(status, iou)
    ...
    NAMELIST /CTRL_EMIS/  l_emis_bio_NO,    &
                          ...
                          l_emis_soil_HONO, & ! MAQ_lg_20170517+ added
                          l_emis_bio_NO_pls,&
                          lcrfyl95,         &
                          iNOemclass,       &
                          zcult,            &
    ...
    WRITE(*,*)  'l_emis_soil_HONO  = ',l_emis_soil_HONO ! MAQ_lg_20170517+ added
	...
	WRITE(*,'(1a,e8.2)')  ' NH3 soil emission flux [mol m-2 s-1]      = ',znh3emis ! MAQ_lg_20230329+
  ...
  PURE SUBROUTINE emdep_emis_bio_VOC( nstep,                & ! ESS_lg_20120722+
    ...
    dm, lad, voc_emfact, tslm1, tcan,                       & ! MAQ_20170505+ added Tcanopy
	...
	! tcan      : canopy temperature profile [K] ! MAQ_20170505+
    ...
    REAL(dp), INTENT(in)  :: dm(:), lad(:,:), voc_emfact(:,:), &
                             tslm1(:), rbvd(:), rvd(:,:), fsl(:,:), tcan(:,:) ! MAQ_20170505+  
  ...
  SUBROUTINE emdep_emis_bio_VOC_MEGAN( nstep,                  &
    ...
    nveglay_hr, nveglay, iisop, imono, iovoc,                  & ! mz_lg_20050725+
    dm, lai, lad, voc_emfact, tslm1, tcan,                     & ! MAQ_20170505+ added Tcanopy
    ...
    REAL(dp), INTENT(in)  :: dm(:), lai(:), lad(:,:), &
							 ...
							 dmbase(:), tcan (:,:)  ! MAQ_20170505+ added Tcanopy MAQ_lg_20160817+ dmbase  
    ...
    DO jl=1,klon
      ...
      DO ii=1,nveglay_hr                    ! loop vertical layers (radiation profile)
        ...
        ! MAQ_20170505+ added the recalculation of the temperature term using the in-canopy temperature profiles 
		x=((1./Topt)-(1./tcan(jl,ii)))/0.00831
        CT=Eopt*(Ct2*EXP(Ct1*x)/(Ct2-Ct1*(1.-EXP(Ct2*x))))        ! equation 15 ACP-6-3181 paper, equation 5, ACPD manuscript on MEGAN

!        print *,'CT',jl,ii,CLsun,CLshade,CT,tcan(jl,ii)
!		 if (pardir(jl)>1000.) read (*,*)
		...
		ELSE
          ! MAQ_20170508+ modified calculation of the temperature dependent MT emissions using CT instead of HEAsun/shade
		  fluxsun(jl,ii,imonoem)=voc_emfact(jl,imonoem)*MEA*DEA*CT* &
               foldenslay(jl,ii)*fsl(jl,ii)
          fluxshade(jl,ii,imonoem)=voc_emfact(jl,imonoem)*MEA*DEA*CT* &
               foldenslay(jl,ii)*(1.-fsl(jl,ii))		! MAQ_lg_20160615-
