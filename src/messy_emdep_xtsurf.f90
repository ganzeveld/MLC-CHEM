MODULE messy_emdep_xtsurf
  ...
  REAL(dp), PUBLIC, PARAMETER :: dtime_mixing=7200._dp ! mz_lg_20050719+ turnover time
...
CONTAINS
  ...
  SUBROUTINE emdep_xtsurf_calcra ( nstep,  & ! ESS_lg_20120722+
    ...
    ! zustveg_obs: friction velocity read in from observations [m s-1]	
    ! 
    ! output 
    ...
    REAL(dp) :: zsurf,zmonin,zoverl,zxzsurf,zxzref,Constm,x  ! MAQ_lg_20230331+ added Constm and x

    REAL(dp),  DIMENSION(:)  , ALLOCATABLE  :: zcdnveg, zcdnslsn, &
          cmveg, cmslsn, cml, psih, psim, psimz0  ! MAQ_lg_20230331+ added psim
    ...
    ALLOCATE(cml(klon))
    ALLOCATE(psih(klon))
	ALLOCATE(psim(klon))   ! MAQ_lg_20230331+ added psim
	ALLOCATE(psimz0(klon)) ! MAQ_lg_20230331+ added psimz0
    ...
    DO jl=1,klon
       ...
       ! MAQ_lg_20230331+ first-order calculation of ustar, needed in the calculation of the stability parameters to
	   !   ultimately arrive at updated ustar as a function of the turbulent properties. This ustar is normally in the online
	   !   version of this model supplied by driver model (SCM/3-D climate model). In the calculations with the SCM, the calculated 
	   !   u* for vegetation (pustveg) (see below) is then having the same value as the pustarl (ustar land) for a vegetation 
	   !   fraction of 1. However, in implementation of that code of the SCM here in MLC-CHEM, there are quite large differences 
	   !   between pustarl and pustveg, with the latter being generally too low for vegetated surface. Consequently, we now
       !   use this first calculation of pustar below as a first order estimate, update then the pustarl to also include the 
       !   the stability effects and then use this value for the pustveg, further then used in the exchange model calculations.
       !   This approach results in a good comparison of the simulated and observed ustar for, for example, the ATTO site.
       ! 
       !   BUT THE REASON WHY IN THIS IMPLEMENTATION A LARGE DIFFERENCE WAS INTRODUCED BETWEEN USTAR AND USTVEG MUST BE STILL RESOLVED!	
       ! MAQ_lg_20230331-	   
       ...
       if (ril(jl).gt.0.) then
          ...
          zmonin=(pustarl(jl)*((tvir(jl)+tvl(jl))/2.)*                      &
               ...
               tvl(jl)))
		  zoverl=zsurf/zmonin
          ! MAQ_lg_20230331+ original calculation of psih, 
          psih(jl)=-4.7*zoverl
          ! MAQ_lg_20230331+ added stability correction function for momentum for stable conditions for calculation of ustar
          psim(jl) = -2./3. * (real(zoverl) - 5./0.35) * exp(-0.35 * real(zoverl)) - real(zoverl) - (10./3.) / 0.35 ! MAQ_lg_20230331+ added psim
		  zoverl=z0m(jl)/zmonin
          psimz0(jl)=-2./3.* (real(zoverl) - 5./0.35) * exp(-0.35 * real(zoverl)) - real(zoverl) - (10./3.) / 0.35 !
       ...
       elseif (ril(jl).lt.0_dp) then
          ...
          ! MAQ_lg_20230331+ added stability correction function for momentum for unstable conditions for calculation of ustar
          ! MAQ_lg_20230331+ added psim (see also CLASS) (alternatively, we should still apply here the PSIM function from the 
		  !    from the SCM (see ec4_vdiff.f) for consistenty with the calculations of PSIH
          x    = (1. - 16. * real(zoverl)) ** (0.25)
          psim(jl) = 3.14159265 / 2. - 2. * atan(x) + log( (1.+x) ** 2. * (1. + x ** 2.) / 8.)
		  zoverl=z0m(jl)/zmonin
		  x    = (1. - 16. * real(zoverl)) ** (0.25)
          psimz0(jl) = 3.14159265 / 2. - 2. * atan(x) + log( (1.+x) ** 2. * (1. + x ** 2.) / 8.)

       else
          ! avoid division by zero in case ril(jl) = 0
          psih(jl)=1.0_dp
          psim(jl)=1.0_dp  ! MAQ_lg_20230331+ added psim
          psimz0(jl)=1.0_dp
          ! mz_lg_20040923-
       endif

       ! MAQ_lg_20230331+ added recalculation of ustar (see also CLASS) for more optimal comparison with observations
       Constm =  ckap**2. / (LOG((geopot_3d(jl)/g)/REAL(az0(jl))) - psim(jl) + psimz0(jl))** 2.
       pustarl(jl) = sqrt(Constm)*SQRT(MAX(pepdu2,um1(jl)**2+vm1(jl)**2))
       ! MAQ_lg_20230331-

       prahl(jl)=MAX(1._dp,(1._dp/(pustarl(jl)*ckap))* &
            (LOG((geopot_3d(jl)/g)/REAL(az0(jl)))-psih(jl)))

	   !print *,'calc',jl,z0m(jl),az0(jl),zsurf,tvir(jl),tvl(jl),&
       !       SQRT(MAX(pepdu2,um1(jl)**2+vm1(jl)**2)),zmonin,psih(jl)
	   ...
	   if (z0m(jl).gt.0..or.loland(jl)) then
          ...
          ! MAQ_AV_20201005+ added observation-based friction velocity, if defined
	      if (zustveg_obs(jl) > 0.) then
            pustveg(jl) = zustveg_obs(jl)
          else
            pustveg(jl) = pustarl(jl) ! MAQ_lg_20230331+ modified; the recalculated ustveg came generally out
                                      ! way too small in comparison with measurements whereas the ustarl used to calculate
                                      ! prahl was substantially larger. Original formula to calculate pustveg (see also SCM):
								      ! SQRT(cmveg(jl))*sqrt(MAX(pepdu2,um1(jl)**2+vm1(jl)**2))
          endif
	      ! MAQ_AV_20201005-
    ...
    DEALLOCATE(cml)
    DEALLOCATE(psih)
	DEALLOCATE(psim) ! MAQ_lg_20230331+
    DEALLOCATE(psimz0)
  ...
  SUBROUTINE emdep_xtsurf_veg_mlay( casename, nstep,           & ! MAQ_lg_20210330+ added casename ! ESS_lg_20120722+
    ...
    l_emis_soil_HONO, l_drydep, l_ags, loland,                 & ! MAQ_20201105+ l_ags, MAQ_lg_20170517+ switch for soil HONO flux
	pfrl, pcvbs, pcvw, pvgrat, zrefsl,                         & ! ESS_lg_20131125+ zrefls
	rahveg, ustveg, u_sl, u_veg,                               & ! ESS_lg_20131125+ u_sl
	netrad, rj, rj_veg, tsurf, tsoil, ws, press, qm1, rh_2m,   & ! MAQ_lg_20170517+ tsoil and ws, ESS_lg_20120725+ added rj, rj_veg, press and qm1 for chemistry
	...
	nh3_emflux, nh3_slflux,                                    & ! MAQ_lg_20230329+ added NH3 vegetation/soil emission flux 
    ...
    ! ----------
    ! input 
    ! casename        : name of specific case studies ! MAQ_lg_20210330+
    ! nstep           : # of timesteps ! ESS_lg_20120722+
    ...
    ! l_emis_bio_jNO3 : switch for biogenic NOx emissions due to NO3 photolysis
    ! l_emis_soil_HONO: switch for soil HONO emissions  ! MAQ_lg_20170517+
    ! l_drydep        : general swith for dry deposition calculations
	! l_ags           :  MAQ_20201105+ l_ags
    ! loland   : land-sea mask, logical
    ! pfrl     : land fraction
	...
	! tsoil    : soil temperature [K]   ! MAQ_lg_20170517+
	! ws       : soil moisture [m]      ! MAQ_lg_20170517+
	...
	! ccompCO2 : CO2 compensation point     ! MAQ_lg_20170720+
    ...
    ! hono_emflux : HONO emissions due to NO3 photodiss [molec. m-2 s-1]
    ! nox_emflux  : NOx emissions due to NO3 photodiss [molec. m-2 s-1]
    ! hono_slflux : soil HONO emissions [molec. m-2 s-1]
	! ddNO3       : nitrate deposition flux [molucules m-2 s-1]
    ! nh3_emflux  : NH3 emissions by vegetation associated with compensation point [molec. m-2 s-1]
    ! nh3_slflux  : soil NH3 emissions [molec. m-2 s-1]
    ! radon_slflux: Radon emission flux [atoms m-2 s-1]
	! co2_slflux  : CO2 soil respiration flux [molec. m-2 s-1]
	! sqt_slflux  : soil SQT emissions [molec. m-2 s-1] 
    ...
    ! MAQ_20170413+
    ! fATERP      : fraction of monoterpene emissions being emitted in the form of a-terpinene
    ! fLIMO       : fraction of monoterpene emissions being emitted in the form of limonene
    ! fMYRC       : fraction of monoterpene emissions being emitted in the form of myrcene
    ! MAQ_20170413-
    ...
    ! Kh_obs_sl   : Eddy diffusivity for heat (and tracers) above canopy, read in from observations [m2 s-1] ! MAQ_AV_20200309+
    ! Kh_obs_cl   : Eddy diffusivity for heat (and tracers) in canopy, read in from observations [m2 s-1] ! MAQ_AV_20200309+
    ! zustveg_obs : Friction velocity over vegetation, read in from observations [m s-1] ! MAQ_AV_20201005+
    !
    ! input/output
    ...
    ! I/O
    INTEGER,  INTENT(in)  :: nstep ! ESS_lg_20120722+
    REAL(dp), INTENT(in)  :: fAPIN, fBPIN, fSQTERP,fATERP, fLIMO, fMYRC ! MAQ_lg_20170413+
    CHARACTER(LEN=20), INTENT(in) :: trname(:)
    LOGICAL,  INTENT(in)  :: l_emis, l_emis_bio_NO, l_emis_bio_VOC, &
                             l_emis_bio_jNO3, l_emis_soil_HONO, l_drydep ! MAQ_lg_20170517+
    LOGICAL,  INTENT(in)  :: latmbios(:), lvd_bigl(:), lexist_GAS(:),  &
                             l_ags, loland(:)                       ! MAQ_20201105+ l_ags 
    ...
    REAL(dp), INTENT(inout)  ::                                   & ! ESS_lg_20130516+ changed to inout to allow overwriting rmes
	  ...
	  rahveg(:), ustveg(:), u_sl(:), u_veg(:,:),                  & ! ESS_lg_20131125+ u_sl
	  netrad(:), tsurf(:), tsoil(:), ws(:), rh_2m(:), lai(:),     & ! MAQ_lg_20170517+ tsoil and ws 
      lad(:,:), hc(:), rco_leaf(:,:), diff(:), diffrb(:), rcut(:),& ! ESS_lg_20130516+ rco_leaf layer dependent
      rmes(:), rws(:), rsoil(:), rmesCO2(:), rcutCO2(:), ccompCO2(:), & ! MAQ_lg_20170720+ added CO2 compensation point ! ESS_lg_20130516+ 
	  fws(:), grvol(:), grmass(:), pdp(:), prhoa(:),              & !       
	  prc(:), prl(:), fsl(:,:), fslsum(:), netradmax, cthru(:),   & ! MAQ_lg_20170526+ extra input to constrain HONO/NOx emission calculations
      NO3s(:,:), HONO_jNO3em(:), NOx_jNO3em(:),                   & !
	  no_slflux(:), no2_slflux(:), isop_emflux(:,:),              & ! MAQ_lg_20210727+ added NO2 soil (deposition) flux for extra diagnostics
      mono_emflux(:,:), ovoc_emflux(:,:), hono_emflux(:,:),       & !
      nh3_emflux(:,:),                                            & ! MAQ_lg_20230423 NH3 vegetation emissions (compensation point calc.)
    ...
    REAL(dp), INTENT(inout)  :: pxtm1(:,:), pxtmveg(:,:,:), psurfflux(:,:), & ! ESS_20120718+ pxtm1 (is updated)
                                ...
                                hono_slflux(:), ddNO3(:,:),       & ! MAQ_lg_20170517+
								nh3_slflux(:), sqt_slflux(:)        ! MAQ_lg_20230601+ added soil SQT emissions, MAQ_lg_20230423 NH3 vegetation emissions
    ...
    CHARACTER(LEN=250)       :: casename     ! MAQ_lg_20210330+

    ! MAQ_AV_20200309+ added declaration of Kh_obs_sl & Kh_obs_cl
    ! MAQ_AV_20201005+ added declaration of zustveg_obs
    REAL(dp), INTENT(inout)  :: Kh_obs_sl(:), Kh_obs_cl(:), zustveg_obs(:)
	REAL(dp), parameter :: dz_obs = 16._dp  ! MAQ_lg_20230605+ added the reference thickness of observed Kh
	INTEGER :: ichange_dz_obs ! MAQ_lg_20230605+ added switch for activating message
    ! MAQ_AV_20200309- 
    ...
    REAL(dp):: dz, dz1, dz2, dz3, ptmst_sub, ts_dd, ts_em,   &
               ...
               rsveg_lay, rswet, dzsl, Kh_sl                    ! MAQ_lg_20210727+ added Kh_sl for use in subroutine messy_emdep_xtsurf_Kh
	...
	REAL(dp) :: rsoilO3_min ! MAQ_lg_20170804+
	...
	ichange_dz_obs=0 ! MAQ_lg_20230605+ added switch for activating message
    ...
    IF (l_emis) THEN
      ...
      DO jl=1,klon
         ...
         ! MAQ_lg_20230329+ added NH3
		 IF (idt_NH3 > 0) THEN
           latmbios_emsoil(idt_NH3)=.TRUE.                
                    emisflux(jl,nveglay,idt_NH3)=nh3_slflux(jl)  ! NH3 soil emissions in molec. m-2 s-1
         END IF
         ! MAQ_lg_20230329-
         ...
         IF ((pvgrat(jl)+pcvw(jl)) > 0._dp) THEN
            ...
            ! MAQ_lg_20230601+ added a soil SQT emission flux
		    IF (idt_SQTERP > 0) THEN
              latmbios_emsoil(idt_SQTERP)=.TRUE. 
              emisflux(jl,nveglay,idt_SQTERP)=emisflux(jl,nveglay,idt_SQTERP)+sqt_slflux(jl)  ! SQT soil emissions in molec. m-2 s-1
            ENDIF
            ! MAQ_lg_20230601-

            ! MAQ_lg_20170413+ added other monoterpenes for ATTO analysis
            ! partioning
            IF (idt_ATERP > 0 .AND. l_emis_bio_VOC) THEN
              latmbios_emveg(idt_ATERP)=.TRUE.  
              emisflux(jl,1:nveglay,idt_ATERP)=fATERP*mono_emflux(jl,1:nveglay)/     &
                        (pvgrat(jl)+pcvw(jl))
            END IF
            IF (idt_LIMO > 0 .AND. l_emis_bio_VOC) THEN
              latmbios_emveg(idt_LIMO)=.TRUE.  
              emisflux(jl,1:nveglay,idt_LIMO)=fLIMO*mono_emflux(jl,1:nveglay)/     &
                        (pvgrat(jl)+pcvw(jl))
            END IF
            IF (idt_MYRC > 0 .AND. l_emis_bio_VOC) THEN
              latmbios_emveg(idt_MYRC)=.TRUE.  
              emisflux(jl,1:nveglay,idt_MYRC)=fMYRC*mono_emflux(jl,1:nveglay)/     &
                        (pvgrat(jl)+pcvw(jl))
            END IF
            ! MAQ_lg_20170413- 
         ...
         DO jt=1,ntrac
           ...
           IF ( (latmbios(jt)) .AND. (lvd_bigl(jt)) ) THEN
             DO jl=1,klon
               ...
               ! MAQ_LC_20200308+ Added variable COS mesophyll conductance from stomatal conductance
               IF (l_xtsurf_AGS.AND.jt.EQ.idt_COS) THEN
				  rmesophyll(jt)=(rco_leaf(jl,2)*diff(jt))*2.6
			   ENDIF
               ! MAQ_LC_20200308-

               ! ESS_lg_20170804+ introduced the O3 soil resistance according to Stella et al.
               IF (jk == 1 .AND. jt == idt_O3) THEN
                 rsoilO3_min=21. ! minimum soil resistance given in s m-1
			     !rsoil(jt)=rsoilO3_min*exp(0.024*100*rh_2m(jl))
               ENDIF
			   ! ESS_lg_20170804-
               ...
               IF (hc(jl) > hcmin .AND. &
                  ...
                  IF (l_ags) rstomx(jl,jt)=diff(jt)*rco_leaf(jl,jk) ! MAQ_lg_20201105+ Ags already considers the soil moisture effect

                  ! lg-    calculation of quasi-laminar boundary layer resistance
                  !        for canopy layers 
				  
				  ! MAQ_lg_2022160516+ see especially the papers by Meyers et al. (HNO3 deposition to a Decideous forest) about a discussion
				  !        about explanation of the terms used in the equation of rbveg where the empirical constant 180 is the value of 455 s^1/2 m^-1 
                  !        divided by factor 2.5 to correct for calculated rbveg that was resulting in leaf-scale fluxes that were a factor 2.5 too small
                  ...
                  ! MAQ_lg_20170720+ added the correction of the CO2 leaf resistance to consider compensation point calculated in Ags model
				  !  (different compared to previously applied approach to consider compensation points for NO2, NH3 etc., see below)
                  IF (l_xtsurf_AGS.AND.jt.EQ.idt_CO2) THEN
                    ! MAQ_lg_20170720+ added the CO2 compensation point
				    ccomp(idt_CO2)=1.e-6*ccompCO2(jl)*1.e-3_dp*prhoa_veg(jk)/(amd/avo) ! mz_lg_20050719+ all the concentrations are recalculated
                                                                                       ! from mixing ratio to molecules cm-3; kg m-3 -> g cm-3 1e-3																                 
					zxtmveg(jk,jt)=pxtmveg(jl,jk,jt)*1.e-3_dp*prhoa_veg(jk)/(amd/avo)
					rleaf(jl,jt)=rleaf(jl,jt)*1./((zxtmveg(jk,jt)-ccomp(idt_CO2))/zxtmveg(jk,jt))
					rleafw(jl,jt)=rleafw(jl,jt)*1./((zxtmveg(jk,jt)-ccomp(idt_CO2))/zxtmveg(jk,jt))
					ccomp(idt_CO2)=0. ! MAQ_lg_20170720+ to avoid including CO2 in the compensation point calculations found below
			      ENDIF
                  ! MAQ_lg_20170720+-		
                  ...
                  IF (jk == nveglay) THEN
                     ...
                     pz(jk)=0.5*hc(jl)/nveglay ! MAQ_lg_20170517+ fixed to deal with other resolution than 4 layers
                     ...
                     IF (rleaf(jl,jt) > 0._dp) THEN  ! dry vegetation
                        rsveg_lay=1._dp/((1._dp/                          & 
                            (rsoil(jt)+rahcan(jl)*pz(jk)/hc(jl)))+        &
                             1._dp/((rbveg(jk,jt)+rleaf(jl,jt))/          & ! MAQ_lg_20170830+ further fix; moved ( to in front of (rbveg(...ESS_lg_20140703+ bug fix, added ) after rleaf(jl,jt)
                             MAX(1.e-5_dp,lad_veglay(jl,jk)*lai(jl))))      ! ESS_lg_20140703+ bug fix, removed here ) after lai(jl)
					 ELSE
                        rsveg_lay=1.e10
                     END IF
                     IF (rleafw(jl,jt) > 0._dp) THEN   ! ESS_LG_20120721+
                       rswet=1._dp/((1._dp/                               & 
                            (rsoil(jt)+rahcan(jl)*pz(jk)/hc(jl)))+        &
                             1._dp/((rbveg(jk,jt)+rleafw(jl,jt))/         & ! MAQ_lg_20170830+ further fix; moved ( to in front of (rbveg(...ESS_lg_20140703+ bug fix, added ) after rleaf(jl,jt)
                             MAX(1.e-5_dp,lad_veglay(jl,jk)*lai(jl))))      ! ESS_lg_20140703+ bug fix, removed here ) after lai(jl)
                     ELSE
                       rswet=1.e10
                     ENDIF
           ...
           IF ( (latmbios(jt)) .AND. lvdaer(jt) ) THEN
             DO jl=1,klon
               ...
               zrint=1.e-8*1.E2  ! um -> CM, mz_lg_20061021+ assumed particle radius 
                                 ! of 1e-2 um
               densaer(jl,jmod)=1.7e3 ! mz_lg_20061021+ assumed density of 1.7 g cm-3
    ...
    IF (nveglay >= 2) THEN
       ...
       DO jl=1,klon
          ...
          ! MAQ_lg_20170517+ calculation of soil and canopy HONO/NOx sources within the time loop since
		  !    these calculations need to the actual concentration to calculated HNO3 deposition and HONO 
		  !    in-canopy concentrations

          IF (idt_HONO > 0 .AND. l_emis_bio_jNO3) THEN
            IF (jl == 1) PRINT *, &
			  'Calculation of the HONO/NOx emission flux from NO3 photolysis in veg_mlay!'
            ...
            IF (jl>1) THEN
			  ddNO3(jl,:)=pxtmveg(jl-1,:,idt_HNO3)*(1e6*1.e-3_dp*prhoa(jl)/(amd/avo))*1./(rahveg(jl))
            ENDIF
			  
            CALL emdep_xtsurf_jNO3( jl,                                            & ! ESS_lg_20120722+
              ptmst, l_xtsurf_veg_mlay, nveglay_hr, nveglay, pcvw, pvgrat,         &
            ...
            IF (idt_NO2 > 0.) THEN
              ...
              emisflux(jl,1:nveglay,idt_NO2)=emisflux(jl,1:nveglay,idt_NO2)+NOx_emflux(jl,1:nveglay)/ &
                       (pvgrat(jl)+pcvw(jl))
            END IF
            ...
            IF (idt_HONO > 0.) THEN
              ...
              emisflux(jl,1:nveglay,idt_HONO)=emisflux(jl,1:nveglay,idt_HONO)+HONO_emflux(jl,1:nveglay)/ &
                       (pvgrat(jl)+pcvw(jl))
            END IF
	      ...
	      ! HONO: soil emissions
          IF (idt_HONO > 0 .AND. l_emis_soil_HONO) THEN
            IF (jl == 1) PRINT *, &
			  'veg_mlay: calculation of soil HONO emission flux using the HONO understorey mixing ratios of previous timestep!'

              pz(nveglay)=0.5*hc(jl)/nveglay
			   
			  IF (jl>1) CALL emdep_xtsurf_soilHONO( jl, prhoa, pz(nveglay), rahveg, ws, tsoil, &
			                                        pxtmveg(jl-1,nveglay,idt_HONO), HONO_slflux) 
			  latmbios_emsoil(idt_HONO)=.TRUE.         
              HONO_emflux(jl,nveglay)=HONO_emflux(jl,nveglay)+HONO_slflux(jl)
		      emisflux(jl,nveglay,idt_HONO)=emisflux(jl,nveglay,idt_HONO)+HONO_slflux(jl)
          END IF
          ...
          IF (hc(jl) > hcmin .AND.   &
			 ...
			 ! MAQ_AV_20200312+ changed surface-layer Kh-value to observation-derived value for Bosco Fontana case
             IF(Kh_obs_sl(jl) <= 0.) THEN
	           Kh(jl,1)= MAX(Khmin,(dz/rahveg(jl))) 
	         ELSE
	           !Kh(jl,1)=MAX(Khmin,Kh_obs_sl(jl))! * (dz / dz_obs.)) !Scaling for the length over which Kh is calculated, i.e. dz between reference heights in model and dz over which Kh is calculated from observations
	           Kh(jl,1)=Kh_obs_sl(jl) * (dz / dz_obs)
               
			   ! MAQ_lg_20230605+ securing to give message on value of dz_obs; to be changed when other than Bosco Fontana dataset is used
	           IF (ichange_dz_obs == 0) THEN
				 print *, 'Kh_sl; use of observed Kh_sl. This one is calculated for an dz_obs for Bosca Fontana case: ',dz_obs
				 print *, 'Change this value in emdep_xtsurf_veg_mlay when Kz_obs/dz_obs represents other dataset: '
                 ichange_dz_obs=1
			     read (*,*)
               ENDIF
			   ! MAQ_lg_20230605+
		     ...
		     DO jk=1,nveglay-1   		! ESS_km_20130221 added loop 
	           ...
	           ! MAQ_AV_20200309+ modified Kh calculation (if Kh_obs is non-zero, those values are used rather than the calculated ones
               IF(Kh_obs_cl(jl) <= 0.) THEN
			     Kh(jl,jk+1)=MAX(Khmin,(dz/(rahcan(jl)/(hc(jl)/dz))))
 			   ELSE
			     Kh(jl,jk+1)=MAX(Khmin,Kh_obs_cl(jl))
               ENDIF
             ...
             DO jt=1,ntrac
                ...
                IF (ccomp(jt) > 0._dp) THEN
				  ...
				  DO jk=1,nveglay
                     ...
                     ! ESS_lg_20070803+   calculation of canopy concentration (at leaf-surface level, not the concentration inside the leaf-stomata!)
                     Ccanopy(jk,jt)=((zxtmveg(jk,jt)/(0.01*rbveg(jk,jt)))+                   & ! Ccanopy in [molec. cm-3]
                       (ccomp(jt)/(0.01*rstomx(jl,jt))))/                                    & ! [molec. cm-2 s-1]
                       (1./(0.01*rbveg(jk,jt))+(1./(0.01*rstomx(jl,jt))+1./(0.01*rcut(jt))))   ! MAQ_lg_20170718+ put extra brackets to stress the
					                                                                           ! the differentation between rbveg and surface conductance term; [molec. cm-2 s-1 (s-1 cm)-1
                     !print *,'Compensation point calculations',jt,trname(jt),Ccanopy(jk,jt)
					 ...
					 ! This term should still be used to calculate properly the dry deposition/emission tendency
                ...
                IF (jt == idt_O3) THEN
                  DO jk=1,nveglay
                    ! MAQ_lg_20190430+ the stomatal resistance was only using the last calculated value for the lowest layer
					!    resulting in a way too small calculated stomatal contribution to the total atmosphere-biosphere flux
					!    Having included again the layer specific rstomx should now give a reasonable estimate of the stomatal 
					!    contribution to the total flux: Note that this bug only had consequences for diagnostics and for the actual
					!    simulation
 				    rstomx(jl,jt)=diff(jt)*rco_leaf(jl,jk)/fws(jl)    ! ESS_lg_20130516+ rco_leaf layer dependent
                    IF (l_ags) rstomx(jl,jt)=diff(jt)*rco_leaf(jl,jk) ! MAQ_lg_20201105+ Ags already considers the soil moisture effect
					...
					! MAQ_lg_20190430+
				    pstomfluxO3(jl,jk)=                                           &
                          -lad_veglay(jl,jk)*lai(jl)*zxtmveg(jk,jt)*1e6*          & ! MAQ_lg_20190430+ added LAD*LAI to get layer scale flux ! [molec. m-3], negative for deposition flux  
				...
				IF (nveglay == 2) THEN
					...
					! MAQ_lg_20210330+ with emisflux(jl,jjk,jt) given in molecules m-2 s-1 and zxtmveg(jjk,jt) 
					!   given in molecules cm-3, the flux is multiplied with 1e-4 to arrive at pvdveg in cm s-1  
				    IF (emisflux(jl,jjk,jt)<0._dp) pvdveg(jl,jjk,jt)=-1e-4*emisflux(jl,jjk,jt)/zxtmveg(jjk,jt)

                    !if (jt.eq.idt_NO2) then
				    !  if (jjk > 0) then
					!    print *,'veg_mlay; emisflux NO2 & Vdveg: ',jl,jt,jjk,emisflux(jl,jjk,jt),zxtmveg(jjk,jt),pvdveg(jl,jjk,jt)
					!    if (pvdveg(jl,jjk,jt).gt.100) read (*,*)
					!  endif
				    !endif
					...
					! MAQ_lg_20210906+ resetting pvdveg to zero for a leaf-scale emission flux, thus when emisflux >=0 (was < 0._dp)  
				    IF (ccomp(jt) > 0 .AND. emisflux(jl,jjk,jt)>0._dp) pvdveg(jl,jjk,jt)=0._dp
             ...
             nox_emflux(jl,nveglay)=emisflux(jl,nveglay,idt_NO2)
             ! ESS_lg_20130124-

           END IF ! IF ( (emisflux_tot(jl,idt_NO) > 0._dp) .AND. &

           ! MAQ_lg_20230329+ NH3
           nh3_emflux(jl,1)=emisflux(jl,1,idt_NH3)
           nh3_emflux(jl,nveglay)=emisflux(jl,nveglay,idt_NH3)
           ! MAQ_lg_20230329-

           ! MAQ_lg_20210727+
    	   ! calculating the NO2 soil deposition flux from the NO2 mixing ratio, recalculated to 
           ! to concentration in molecules m-3 and multiplied with the Vdsoil NO2 in m s-1, calculated here
           ! as 1/(Rsoil + Rahcan * dz/canopy height) with Rsoil in s m-1                
           IF (idt_NO2 > 0) THEN
			 NO2_slflux(jl)=-(pxtmveg(jl,nveglay,idt_NO2)*(1e6*1.e-3_dp*prhoa(jl)/(amd/avo))* &   ! MAQ_lg_20210826+ minus sign; a negative flux reflects deposition/removal from understorey
	              1./(rsoil(idt_NO2)+rahcan(jl)*pz(nveglay+1)/hc(jl)))
           ENDIF
           ! MAQ_lg_20210727-
  ...
  SUBROUTINE emdep_xtsurf_calc_rs( &
    ...
    ! mz_lg_20030811+, O3
    diffrb_o3=1.28_dp  ! MAQ_lg_20220908+ modified from 1.2 to 1.28 based on having done once the calculation with the explicit formula
    rsoil_o3=400._dp
    rwater_o3=2000._dp
    rws_o3=2000._dp
    ...
    DO jt=1, ntrac
     ...
     CASE DEFAULT
       ...
       IF (lo_derived .AND. (lvd_bigl(jt))) THEN
          ...
          ! diff BL=(v/Dx)**2/3, with v (thermal diffusivity of air) = 0.189 cm2 s-1 and 
          ! Dx (molecular diffusivity of compound X) = Dh2o/sqrt(Mx/Mh2o), with Dh2o = 0.212 cm2 s-1
		  ! See also Hicks et al., WATS, 1987

          diffrb(jt)=(0.189/(0.212/diff(jt)))**(2./3.)

          ! calculation of rmx, the mesophyllc resistance
		...
		!IF (TRIM(trname(jt))=='APIN' .OR. TRIM(trname(jt))=='BPIN' .OR. TRIM(trname(jt))=='LIMO' .OR. TRIM(trname(jt))=='COS') then 

        ! MAQ_LC_2019+
		IF (TRIM(trname(jt))=='COS') THEN
			rws(jt)=1e5_dp
			rmes(jt)=4.7e3_dp 			! MAQ_LC_20200109+ Increased rmes for COS, was 1, based on measured data from 2017
			rsoil(jt)=5.1e3_dp			! MAQ_LC_20191018+ Changed to constant soil resistance following Sun et al. 2018 and from measurements
			diffrb(jt)=1.5_dp
			diff(jt)=2._dp				! MAQ_LC_20190203+ from Seibt et al., 2010 (or 1.8 when calculated as above)
			!rwater(jt)= 1e5			! MAQ_LC_20191023+ Changed to constant water uptake resistance -> not used
		ENDIF
        ! MAQ_LC_2019-

		IF (TRIM(trname(jt))=='O3'.OR.TRIM(trname(jt))=='COS') then
	  	   write (*,'(1a,1a,1a,e8.2,1a,e8.2,1a,e8.2)') &
	         ' Calc. resistances for: ',TRIM(trname(jt)),' rws: ', rws(jt),' rsoil: ',rsoil(jt),' rmesophyll: ',rmes(jt)
  ...
  SUBROUTINE emdep_xtsurf_calcprof( nstep, & ! ESS_lg_20120722+
    nveglay_hr, globrad, cossza_2d, lai, lad, rbvd, rvdsl, rvd, fsl) ! MAQ_lg_20201016+ globrad instead of netrad ! mz_lg_20050721+
    ! ---------------------------------------------------------------
    ...
    ! globrad   : ! MAQ_lg_20201016+ globrad instead of netrad [W M-2]
    ...
    REAL(dp), INTENT(in)  :: globrad(:), cossza_2d(:), lai(:), lad(:,:) ! MAQ_lg_20201016+ globrad instead of netrad
    ...
    DO jl=1,klon
       rg=globrad(jl)	   ! MAQ_lg_20201016+ globrad instead of netrad
	   !print *,'calcprof: ', rg
  ...
  SUBROUTINE emdep_xtsurf_canrad2(n,pai,zen,rg,fsl,rvd,rbvd,rvdsl,parbeam)
    ...
    ! MAQ_lg_20201016+ potential bugfix based on critical evaluation of Weiss and Norman paper and code by Peter Bosman
	! who found a much improved representation of in-canopy PAR simulations for Hyytiala
    ! correction of equation 3 of that paper. 	
    !RFVIS=0.4*(600*COS(ZEN)-RDVIS)
    ! MAQ_lg_20201016-
    ...
    ! MAQ_lg_20201016+ potential bugfix based on critical evaluation of Weiss and Norman paper and code by Peter Bosman
	! who found a much improved representation of in-canopy PAR simulations for Hyytiala
    ! correction of equation 5 of that paper. 	
    !RFIR=0.60*((720.-WA)*COS(ZEN)-RDIR)
    ! MAQ_lg_20201016-
...
100 CPAI = 0.0_dp
  ...
  SUBROUTINE emdep_xtsurf_ccomp(nstep,prhoa,vgrat,tsurf,pxtmveg_CO2,idt_CO2, &
    ...
    DO jl=1,klon
      ...
      !ccomp(idt_CO2)=100.*1e-3*prhoa(jl)/(amd*1e6/avo) ! 100 ppmv to molecules cm-3 ! alternative 0.99*pxtmveg_CO2(jl)   

	  ccomp(idt_NO2)=2.5*1e-3*prhoa(jl)/(amd*1e9/avo)  ! 2.5 ppbv to molecules cm-3
  ...
  SUBROUTINE emdep_xtsurf_chem( istep, nstep, ntrac,          &  
   ...
   &     RSO4NH3(NLON),RMAPINOH(NLON),RMBPINOH(NLON),RSQTERPOH(NLON),  &
   &     RMAPINO3(NLON),RMBPINO3(NLON),RSQTERPO3(NLON),                &
   &     RMAPINNO3(NLON),RMBPINNO3(NLON),RSQTERPNO3(NLON),             &

         ! MAQ_lg_20170413+    
   &     RMATERPOH(NLON),RMLIMOOH(NLON),RMMYRCOH(NLON),                &
   &     RMATERPO3(NLON),RMLIMOO3(NLON),RMMYRCO3(NLON),                &
   &     RMATERPNO3(NLON),RMLIMONO3(NLON),RMMYRCNO3(NLON),             &
         ! MAQ_lg_20170413-  
   ...
   &     ACID(NLON),MAPIN0(NLON),MBPIN0(NLON),SQTERP0(NLON),           & ! MAQ_lg_20170413+ modified names a/b-pinene
   &     MATERP0(NLON),MLIMO0(NLON),MMYRC0(NLON),                      & ! MAQ_lg_20170413+ added monoterpenes for ATTO tower analysis
   &     HONO0(NLON),CH2OHO2H0(NLON),RCHOHO2H0(NLON),                  &
   &     MAPIN(NLON),MBPIN(NLON),SQTERP(NLON),HONO(NLON),              & ! MAQ_lg_20170413+ modified names a/b-pinene
   &     MATERP(NLON),MLIMO(NLON),MMYRC(NLON),                         & ! MAQ_lg_20170413+ added monoterpenes for ATTO tower analysis
   ...
   &     PNH2,XLNH2,XLNH3,PMSA,PMAPIN,XLMAPIN,PMBPIN,XLMBPIN,          &   ! MAQ_lg_20170413+ modified names for a/b-pinene
   ...
   &     PMATERP,XLMATERP,PMLIMO,XLMLIMO,PMMYRC,XLMMYRC                    ! MAQ_lg_20170413+
 ...
 1102 CONTINUE
      ...
      DO 1103 JL=1,NLON
        ...
        RJHONO(JL)=0.189*RJNO2(JL)+8.433E-2*RJNO2(JL)**2  ! MAQ_20170529+ enhanced rj HONO for sensitivity test: factor 10 increase?

!        RJHONO(JL)=10.*(0.189*RJNO2(JL)+8.433E-2*RJNO2(JL)**2)  ! MAQ_20170529+ enhanced rj HONO for sensitivity test: factor 10 increase?
!        IF (jk ==1 .AND. istep==1) then 
!		  print *,'Chemistry scheme: line 2838 in emdep_xtsurf.f90: factor 10 increase in HONO photolysis rate'
!		  print *,'ENTER to continue'
!		  read (*,*)
!		ENDIF
 ...
 1103 CONTINUE
     ...
     &     (H2O(JL)*RH2OOD(JL)+AIR(JL)*RODM(JL))
        ...
        RMAPINOH(JL)=(1.+IS(72,JK))*ZFARR(12.1E-12,444.,ZTREC)
        RMBPINOH(JL)=(1.+IS(73,JK))*ZFARR(23.8E-12,357.,ZTREC)
        RMAPINO3(JL)=(1.+IS(74,JK))*ZFARR(1.01E-15,-732.,ZTREC)
        RMBPINO3(JL)=(1.+IS(75,JK))*1.5E-17
        RMAPINNO3(JL)=(1.+IS(76,JK))*ZFARR(1.19E-12,490.,ZTREC)
        RMBPINNO3(JL)=(1.+IS(77,JK))*2.51E-12
		...
		! MAQ_lg_20170413+ added the monoterpenes for ATTO tower measurement analysis
		! see spreadsheet provided by Ana Serrano for overview of these reactivities and references
		RMATERPOH(JL)=2.32e-10   ! 
        RMATERPO3(JL)=2.1e-14    !  
        RMATERPNO3(JL)=1.4e-10   ! 
		RMLIMOOH(JL)=1.64e-10    ! 
        RMLIMOO3(JL)=2.1e-16     ! 
        RMLIMONO3(JL)=1.2e-11    ! 
		RMMYRCOH(JL)=3.34e-10    ! 
        RMMYRCO3(JL)=4.7e-16     !   
        RMMYRCNO3(JL)=1.0e-15    ! still needs to be defined		
		! MAQ_lg_20170413-
 ...
 1104 CONTINUE
      ...
      DO 1105 JL=1,NLON
        ...
        MAPIN0(JL)=ZPM(JL,jk,idt_apin)
        MBPIN0(JL)=ZPM(JL,jk,idt_bpin)
        MTTERP0(JL)=ZPM(JL,jk,idt_mtterp) ! mz_lg_20060403+ extra monoterpene

		! MAQ_lg_20170413+
		MATERP0(JL)=ZPM(JL,jk,idt_aterp)
        MLIMO0(JL)=ZPM(JL,jk,idt_limo)
        MMYRC0(JL)=ZPM(JL,jk,idt_myrc) 
	    ! MAQ_lg_20170413-
     ...
     &            PAN(JL)+MPAN(JL)+NITR(JL)
        ...
        ! MAQ_lg_20170413+ added monoterpenes for ATTO tower measurements
 ...
 9999     CONTINUE
     ...
     &      RMAPINO3(JL)*MAPIN(JL)+RMBPINO3(JL)*MBPIN(JL)+ &
	 &      RMATERPO3(JL)*MATERP(JL)+RMLIMOO3(JL)*MLIMO(JL)+RMMYRCO3(JL)*MMYRC(JL)+ & ! MAQ_20170413+
     ...
     &       0.4*RMPANOH(JL)*MPAN(JL)
          ...
          RTERPO3= &
     &         0.76*RMAPINO3(JL)*O3(JL) &
     &        +0.33*RMBPINO3(JL)*O3(JL) &
     ...
     &      +0.76*RMAPINO3(JL)*MAPIN(JL)*O3(JL) &
     &      +0.33*RMBPINO3(JL)*MBPIN(JL)*O3(JL) &
     ...
     &      +FXO2_ISOP*0.75*RXO2HO2(JL)*XO2(JL)*HO2(JL) ! MAQ_20170510+ previously the yield was wet at 4.5! now reduced again to 0.75
     ...
     &      +RMAPINOH(JL)*MAPIN(JL) &
     &      +RMBPINOH(JL)*MBPIN(JL) &
     &      +RMTTERPOH(JL)*MTTERP(JL) & ! mz_lg_20060403+ 

	      ! MAQ_lg_20170413+ 

     &      +RMATERPOH(JL)*MATERP(JL) &
	 &      +RMLIMOOH(JL)*MLIMO(JL) &
	 &      +RMMYRCOH(JL)*MMYRC(JL) &
     ...
     &      RMC23HO2(JL)*HO2(JL)
          ...
          PCH2OHO2H=0.105*RMBPINO3(JL)*MBPIN(JL)*O3(JL)
          ...
          PRCHOHO2H=0.05*RMAPINO3(JL)*MAPIN(JL)*O3(JL)+ &
     &           0.23*RMBPINO3(JL)*MBPIN(JL)*O3(JL)
     ...
     &      RXO2NNO(JL)*XO2N(JL)*ZNO(JL)
          ...
          PMAPIN=0.
          XLMAPIN=RMAPINOH(JL)*OH(JL)+RMAPINO3(JL)*O3(JL)+ &
     &      RMAPINNO3(JL)*ZNO3(JL)
          MAPIN(JL)=(MAPIN0(JL)+PMAPIN*DT)/(1.+XLMAPIN*DT)
          PMBPIN=0.
          XLMBPIN=RMBPINOH(JL)*OH(JL)+RMBPINO3(JL)*O3(JL)+ &
     &      RMBPINNO3(JL)*ZNO3(JL)
          MBPIN(JL)=(MBPIN0(JL)+PMBPIN*DT)/(1.+XLMBPIN*DT)
     ...
     &      RMTTERPNO3(JL)*ZNO3(JL)
          ...
          ! MAQ_lg_20170413+ added extra monoterpene for ATTO tower analysis
     ...
     &      RMATERPNO3(JL)*ZNO3(JL)
          ...
          PMLIMO=0.
          XLMLIMO=RMLIMOOH(JL)*OH(JL)+RMLIMOO3(JL)*O3(JL)+ &
     &      RMLIMONO3(JL)*ZNO3(JL)
          MLIMO(JL)=(MLIMO0(JL)+PMLIMO*DT)/(1.+XLMATERP*DT)			  

          PMMYRC=0.
          XLMMYRC=RMMYRCOH(JL)*OH(JL)+RMMYRCO3(JL)*O3(JL)+ &
     &      RMMYRCNO3(JL)*ZNO3(JL)
          MMYRC(JL)=(MMYRC0(JL)+PMMYRC*DT)/(1.+XLMMYRC*DT)			  
 ...
 2104 CONTINUE 
      ...
      DO 106 JL=1,NLON
        ...
        ZPM(JL,JK,idt_APIN)=MAPIN(JL)
        ZPM(JL,JK,idt_BPIN)=MBPIN(JL)
        ZPM(JL,JK,idt_MTTERP)=MTTERP(JL) 

        ! MAQ_lg_20170413+
        ZPM(JL,JK,idt_ATERP)=MATERP(JL)
        ZPM(JL,JK,idt_LIMO)=MLIMO(JL)
        ZPM(JL,JK,idt_MYRC)=MMYRC(JL) 
        ! MAQ_lg_20170413-
        ...
        OHreact(istep,JK)=(                            &
               ...
               RMAPINOH(JL)*ZPM(JL,JK,idt_apin)+       &
               RMBPINOH(JL)*ZPM(JL,JK,idt_bpin)+       &
               RMTTERPOH(JL)*ZPM(JL,JK,idt_mtterp)+    &

			   ! MAQ_20170413+
               RMATERPOH(JL)*ZPM(JL,JK,idt_aterp)+       &
               RMLIMOOH(JL)*ZPM(JL,JK,idt_limo)+       &
               RMMYRCOH(JL)*ZPM(JL,JK,idt_myrc)+    &
			   ! MAQ_20170413-			   
  ...
  SUBROUTINE emdep_xtsurf_readdata(infilename,nstop,nprint,dtime,ndtgact,ldatltime,readdata)  ! ESS_lg_20130228+ ndtgact
     ...
     PARAMETER (NDATA_MAX=200000,NPARAM_MAX=100,NUNDATA=2,NUNOBS=3) ! MAQ_20201124+ increased to 200000, ESS_lg_20100625+ increased to deal with larger files
     ...
     CHARACTER*1000 DUMMY ! MAQ_lg_20170512+ enhanced length of sentence
   ...
   101   CONTINUE
         ...
         READ(NUNDATA,'(A1000)') DUMMY ! MAQ_20170512+ enhanced length of line
         READ(NUNDATA,'(A1000)') DUMMY

         N=0
         IIP=0
   77    CONTINUE        
         IP=INDEX(DUMMY(IIP+1:1000),' ')  ! MAQ_20170512+ enhanced length of line
...
!         read (*,*)
   ...
   200   PRINT *,'error reading data file, check file',i  ! in case of error
         STOP
   ...
   300   NDATA=I-1                            ! IN CASE OF EOF
       ...
       !     observations is continuously corrected for the evolving time of the simulation by
  ...
  SUBROUTINE emdep_xtsurf_rjveg( nstep, & ! ESS_lg_20120722+
    ...
    REAL(dp):: fsl_avg(nveglay+1),rvd_avg(nveglay+1)  ! MAQ_lg_20180731+ added 1 element to determine layer average fsl_avg/rvd_avg 
        ...
        DO jl=1,klon
          IF (hc(jl) > hcmin .AND. lai(jl) > 1.e-10_dp) THEN
            DO jk=1,nveglay
              ...
              ! MAQ_lg_20180731+ evaluation of the higher resolution (> 2 layers) version of the model
			  !    indicates that the photolysis rates of the top-canopy layer is exactly the same as those of 
			  !    of the surface layer, which is not "realistic"; this is because rvd_avg(jk) for the
			  !    the top layer is the same as rvdsl (diffusive radiation term in the surface layer)
			  !    In order to estimate the photolysis rate in each layer representative for the reference height
			  !    it seems more appropriate to use the average of the fsl/rvd terms of the top layer and the layer
			  !    below to determine the j-value at the reference height of the layer doing a simple averaging of the 
              !    value at the top of the layer and the one at the top of the next layer.			  
			  
              IF (rvdsl(jl) > 1e-10) &     ! MAQ_lg_20230405: with the modified calculation of DayReal/rj; at the end of the day with rvdsl = 0, rj_veg showed a peak...
                rj_veg(jl,jk,jt)= MAX(0._dp,                                &
		   	      ((1.-(fsl_avg(jk)+fsl_avg(jk+1))/2.)*                     & ! fraction of shaded leaves in each layer
				  ((rvd_avg(jk)+rvd_avg(jk+1))/2.)/MAX(1.e-2_dp,rvdsl(jl))+ & ! relative ratio of diffuse radiation in each layer compared to SL
				  (fsl_avg(jk)+fsl_avg(jk+1))/2.)     *                     & ! fraction of sunlit leaves in each layer; 1 / 1 ratio of direct comp.
                  rj(jl,jt))                                                  ! surface layer photolysis rate

 	    	  ! MAQ_lg_20180731+ alternative approach to estimate the in-canopy photolysis rates
			  !    using only the fraction of sunlit leaves assuming that photolysis is controlled by the
			  !    direct beam component of radiation
			  !rj_veg(jl,jk,jt)= &
              !   MAX(0._dp,fsl_avg(jk)*rj(jl,jt))	 
  ...
  SUBROUTINE emdep_xtsurf_ags(nstep, nveglay,   Agstype,     & ! Ags vegetation type, C3; Agstype=1, C4; Agstype=2, conifereous forest; Agstype=3, tropical forest; Agstype=4
	                          ...
	                          PSRFL,   RBVD,    RVD, FSL,    & !
							  PRS0,    PRS0_AGSML,           & !
							  RMESCO2, RCUTCO2, CCOMPCO2)	   ! MAQ_lg_20170720+ added the CO2 compensation point  
      IMPLICIT NONE 																	
	  ...
	  REAL(dp), INTENT(out) :: PRS0_AGSML(:,:),PRS0(:),RMESCO2(:),RCUTCO2(:),CCOMPCO2(:) ! MAQ_lg_20170720+ added the CO2 compensation point 
      ...
      ! MAQ_20211230+ extra declarations for important bug fix; using the lower resolution average radiation fields!
      REAL(dp):: fsl_avg(nveglay),rvd_avg(nveglay)
      INTEGER :: jjk
      ! MAQ_20211230+
						
      REAL(dp) C25,C10,CDOT3,C9,C1DOT6,CMA,CMV,CMC02,C1000
      REAL(dp) CCS,CEXPAR,ZPARMAX, ESATSURF, EFINAL ! MAQ_lg_20201025+ added ESATSURF + EFINAL
      ...
      ! MAQ_lg_20170717+ the CO2 compensation point concentration expressed in ppmv
      CGAMMA25(1)=45.
      CGAMMA25(2)=2.8
      CGAMMA25(3)=45.
      CGAMMA25(4)=100. ! ESS_lg_20150624+ tropical rainforest, higher Ci compared to C3 vegetation
	                   ! is this also consistent with lower Vcmax?
      ...
      ! MAQ_lg_20170716+ inclusion of units to add clarification of this code implementation
	  ! CGM25 is the minimum mesophyll conductance at 25 degrees C; its value resembles gm as defined in the 
	  ! the KNMI report by Steeneveld; [mm s-1]
      ...
      CGM25(4)=1.0      ! ESS_lg_20150624+ tropical rainforest, much lower mesophyll conductance at 25C; the selected
	                    !   value could be calculated such that that mesophyll resistance results in VdCO2 for tropical forest
						!   of ~0.12 cm s-1 (inferred from an observed maximum atmosphere-biosphere flux of ~20 umol m-2 s-1
						!   and a typical CO2 concentration of 380-400 ppmv which resembles about 700-750 mg CO2 m-3. However,
						!   the CO2 exchange also strongly depends on the Ci which is substantially > 0.
	  
      ! MAQ_lg_20170716+ term that describes how gm changes with a change in temperature (Q10...)
      CGMQ10(1)=2.0
      CGMQ10(2)=2.0
      CGMQ10(3)=2.0
      CGMQ10(4)=1.25    ! ESS_GK_20150407 Lowered from 2.0 to 1.25 to simulate more typical tropical forest plant behavior
	  
      CDOT3=.3

      ! MAQ_lg_20170716+ temperature correction term gm; minimum
      CGMT1(1)=5.
      CGMT1(2)=13.
      CGMT1(3)=5.
      CGMT1(4)=10.      ! ESS_lg_20150624+ tropical rainforest, see BSc thesis study Gijs Koetsenruijter, July 2015
	  
      ! MAQ_lg_20170716+ temperature correction term gm; maximum
      CGMT2(1)=28.
      CGMT2(2)=36.
      CGMT2(3)=28.
      CGMT2(4)=38.      ! ESS_lg_20150624+ tropical rainforest, see BSc thesis study Gijs Koetsenruijter, July 2015
	  
      ! MAQ_lg_20170717+ cuticular conductance for CO2 [mm s-1]
      CGC(1)=.25 !.01	    ! gmin,c, see report by van de Kassteele
      CGC(2)=.25
      ...
      ! MAQ_lg_20170716+ inclusion of units to add clarification of this code implementation:[-]
      ! CF0 represents the ratio of the internal concentration (Ci) to the ambient concentration (Cs)	  
	  CF0(1)=.89  ! 0.6   ! Harvard forest simulations .89	  
      CF0(2)=.85
      ...
      CF0(4)=.98        ! ESS_lg_20150624+ tropical rainforest, taking similar values as C3 vegetation
	                    ! except of specific modifications given below
      ...
      ! MAQ_lg_20170717+ prescribed CO2 mixing ratio; the model actually uses a prescribed SL CO2 mixing ratio expressed by PXTMCO2
      CCS=380.

      ! MAQ_lg_20170717+ light use efficiency [mg J-1]
	  CEPSILON0(1)=.017
      CEPSILON0(2)=.014
      ...
      ! MAQ_lg_20170716+ inclusion of units to add clarification of this code implementation
	  ! maximum photosynthetic rate at 25 degrees C: [mg m-2 s-1]
      ...
      CAMAX25(4)=1.   ! MAQ_lg_20170717+ using a value of 1, decreasing this value from 2.2 to 1 does not induce a large difference ! ESS_lg_20150624+ tropical rainforest
      ...
      ! MAQ_lg_20170717+ some more constants
	  C9=9.
      C1DOT6=1.6      ! MAQ_lg_20170716+ correction term for differences in diffusivity CO2 and H2O
      CMA=28.9
      CMC02=44.
      C1000=1000.
      CMV=18.
	  ...
	  DO JL=1,klon
        ...
        ZGAMMA=CGAMMA25(K)*CGAMMAQ10(K)**(((PTSM1M(JL)-TMELT)-C25)/C10)  ! MAQ_lg_20170717+ Equation A4 in KNMI report #242, Steeneveld

		CCOMPCO2(JL)=ZGAMMA   ! MAQ_lg_20170720+ added the assignment of the CO2 compensation point in ppmv 

        ZGAMMA=ZGAMMA*(CMC02/CMA)*ZRO

        ! MAQ_lg_20170717+ mesophyll conductance corrected for T
	    ZGM=(CGM25(K)*CGMQ10(K)**(((PTSM1M(JL)-TMELT)-C25)/C10)) / &     ! MAQ_lg_20170717+ Equation A5 in KNMI report #242, Steeneveld
        ...
        ! LG- end
        ! MAQ_lg_20170717+ maximum photosynthetic rate corrected for T
        ZAMAX=  &
         (CAMAX25(K)*CAMAXQ10(K)**(((PTSM1M(JL)-TMELT)-C25)/C10)) / &    ! MAQ_lg_20170717+ Equation A5 in KNMI report #242, Steeneveld
        ...
        ZSOIL=2.*ZSOIL-ZSOIL**2.   ! MAQ_lg_20220713+ this function makes that ZSOIL shows a somewhat reduced sensitivity to a decrease in 
		                           ! soil moisture for wsm1m < 0.75 wsmax compared to the function applied in the SCM/ECHAM (which shows a linear decrease) 
        ZGM=ZGM/C1000 
        ...
        ! MAQ_lg_20170720+ ZDS=vapour pressure deficit at the leaf level: [kPa]
	    ZDS=(PQS(JL)-PQSAM(JL))*C1000  ! Ags-CLASS Ds       = (esatsurf - efinal) / 1000.0 ! in kPa

        ZDS=PQS(JL)/(0.622+0.378*PQS(JL))  &
         -PQSAM(JL)/(0.622+0.378*PQSAM(JL))
		ZDS=ZDS*PAPHM1(JL)/C1000

        ! MAQ_lg_20201025+ To check the orginal calculation of Ds: added the calculation of ESATSURF and EFINAL as done in CLASS-Ags 
		!    using the commonly available parameters PTSM1M and PQM1 in MLC-CHEM
        ESATSURF = 0.611e3 * exp(17.2694 * (PTSM1M(JL) - 273.16) / (PTSM1M(JL) - 35.86)) ![Pa]
        EFINAL   = PQM1(JL)* (PAPHM1(JL)) / 0.622    ! [Pa] with PQM1 given in g g-1, removed 1e-3
        ZDS      = (ESATSURF - EFINAL) / 1000.0      ! in kPa

        ZDSMAX=(ZFMIN-CF0(K))/CRICO(K) ! MAQ_lg_20201025+ see Ags-CLASS and Steeneveld KNMI report: (F0-FMIN)/Ad, but in Ags Ad =0.07
	                                   ! whereas in the SCM/MLC-CHEM implementation CRICO is = -0.07....
        ZDS=MIN(ZDSMAX,MAX(ZDS,0.))

        !print *,'Ds: ',1000*pqm1(jl),ptsm1m(jl),PAPHM1(JL),esatsurf,efinal,zds
		!if (ptsm1m(jl).gt.293.) read (*,*)
        ...
        ZCI=ZF*ZCS+(1-ZF)*ZGAMMA                           ! Ci [mg m-3]
        ...
        ZAN=((ZAM+ZRD)/(CEXPAR*PVLTM(JL)))*(1.-EXP(-ZB))* &  ! MAQ_lg_20170716+ ZAM is the assimilation term and ZRD the dark respiration term
          (1.-EXP(-CEXPAR*PVLTM(JL)))
	    ZA1=1./(1.-CF0(K))
        ZDSTAR=(1./(ZA1-1.))*ZDSMAX
        ZGSC=PVLTM(JL)*(CGC(K)/C1000  &                      ! MAQ_lg_20170716+ calculation of CO2 leaf conductance according to equation 2.12
          +ZA1*ZSOIL*ZAN/((ZCS-ZGAMMA)*(1.+ZDS/ZDSTAR)))     ! of KNMI report #242 by Steenveld; the term CGC(K)/C1000 = gmin,c and the second term
		                                                     ! includes the actual assimilation component
		ZGS=C1DOT6*ZGSC  ! MAQ_lg_20170716+ water vapor stomatal conductance!

        PRS0(JL)=1./(ZGS)                       ! MAQ_lg_20170716+ water vapor stomatal resistance!
        ...
        IF (l_xtsurf_veg_mlay) THEN
          ...
          DO JK=1,nveglay
            ...
            ! MAQ_20211230+ important bug fix; using the lower resolution average radiation fields!
            ...
            ZCS=PXTMCO2(JL)*(CMC02/CMA)*ZRO ! ESS_lg_20101221+ PXTMCO2 in ppmv, note that this is the surface layer mixing ratio
			                                ! to actually use the calculated in-canopy CO2 mixing ratios, the call to this routine has to 
                                            ! be included in the subroutine veg_mlay
            ...
            ! MAQ_20211230+ FSL_AVG and RVD_AVG
            ZPAR=MAX(ZEPSR,(RBVD(JL)*FSL_AVG(JK)+  &   
	           RVD_AVG(JK)*(1-FSL_AVG(JK))))       ! MAQ_lg_20170717+ REMOVED CVRAD: RBVD/RVD are already the PAR components ! ESS_lg_20140313+ added (1-FSL(JL,JK))
            ! MAQ_20211230-
		    ...
		    ZA1=1./(1.-CF0(K))
            ZDSTAR=(1./(ZA1-1.))*ZDSMAX
            ZGSC=1.*(CGC(K)/C1000  &	! LAI of 1 (PVLTM)    ! MAQ_lg_20170716+ calculation of CO2 leaf conductance according to equation 2.12
               +ZA1*ZSOIL*ZAN/((ZCS-ZGAMMA)*(1.+ZDS/ZDSTAR))) ! of KNMI report #242 by Steenveld; the term CGC(K)/C1000 = gmin,c and the second term
		                                                      ! is the second term including the actual assimilation component
            ZGS=C1DOT6*ZGSC

            !if (zpar > 10) then
			!print *,jk,zpar,100*zgsc,zsoil,zan,zcs,zgamma,zds,zdstar,zdsmax,zam,zrd,cexpar,zb,zepsilon,PTSM1M(JL),TMELT
		    !read (*,*)
            !endif

!  		    if (zpar > zparmax-0.001*zparmax) then
!		      print *,'AGS, multi-layer: ',jk,jl,zpar,PTSM1M(JL),100.*zgsc,ZA1,ZAN,ZCS,ZGAMMA,ZDS,ZDSTAR,ZDSMAX,CGC(K) & ! ZB,ZAMAX,ZGM,ZCI ! ZCI-ZGAMMA,(1-EXP((-ZGM*(ZCI-ZGAMMA))/ZAMAX)), &
!		      print *,'AGS, multi-layer: ',jk,jl,zpar,PTSM1M(JL),100.*zgsc,zam,zrd,cexpar,zb,zepsilon,zcs,zgamma
!		      print *,'AGS, multi-layer: ',jk,jl,zpar,PTSM1M(JL),100.*zgsc,zamax,zgm,zci,zf,zfmin,zsoil
!			  ,RMESCO2(JL)
!		      read (*,*)
!		    endif
            ...
            ! MAQ_20211230+ added threshold on low temperatures based on Borden case simulation
            IF (PSRFL(JL).LT.0.1.OR.PVLTM(JL).LT.0.01.OR.PTSM1M(JL).LT.273.) PRS0_AGSML(JL,JK)=1.E5
  ...
  SUBROUTINE emdep_xtsurf_jNO3( Nstep,                                  & ! ESS_lg_20120722+
	...
	! netrad    : net solar radiation ! MAQ_lg_20170524+
	! netradmax : maximum net solar radiation ! MAQ_lg_20170524+
    ...
    ! I/O
    REAL(dp), INTENT(in)    :: delta_time, netradmax ! MAQ_lg_20170524+ netradmax
    ...
    REAL(dp), INTENT(in)    :: pvgrat(:), pcvw(:),                  &
                               prc(:), prl(:), fsl(:,:), netrad(:), & ! MAQ_lg_20170524+
                               lad(:,:), lai(:), ddNO3(:,:)            
    ...
    ! MAQ_lg_20170526+ modified to deal with calling this subroutine within the timeloop of veg_mlay.f
    DO jl=klon,klon ! jl=1,klon
      ...
      IF (jl>1) THEN ! ESS_lg_20130516+ modified based on bounds check
        IF (l_xtsurf_veg_mlay) THEN ! multilayer model approach
          ...
          DO jk=1,nveglay  
             ...
             DO jjk=(jk-1)*nveglay_hr/nveglay+1,jk*nveglay_hr/nveglay
               HONO_emflux(jl,jk)=    &
                   lad(jl,jjk)*lai(jl)*fsl(jl,jjk)*(netrad(jl)/netradmax)*jNO3s_HONO*NO3s(jl-1,jk)  ! MAQ_lg_20170524+ added scaling with netrad
               NOx_emflux(jl,jk)=     &
                   lad(jl,jjk)*lai(jl)*fsl(jl,jjk)*(netrad(jl)/netradmax)*jNO3s_NOx*NO3s(jl-1,jk)   ! MAQ_lg_20170524+ added scaling with netrad
             END DO
          END DO
          ! mz_lg_20050725-
       ...
       &       lai(jl)*fslsum(jl)*(netrad(jl)/netradmax)*jNO3s_HONO*NO3s(jl-1,1)  ! MAQ_lg_20170524+ added scaling with netrad
          NOx_jNO3em(jl)=(1.-pcvw(jl))*pvgrat(jl)*    &
               lai(jl)*fslsum(jl)*(netrad(jl)/netradmax)*jNO3s_NOx*NO3s(jl-1,1)   ! MAQ_lg_20170524+ added scaling with netrad
        ...
        IF (l_xtsurf_veg_mlay) THEN
            NO3s(jl,:)=NO3s(jl-1,:)                            &
               +ddNO3(jl,:)*delta_time                         &  ! ESS_lg_20130117+ to accumulate, added by dry deposition
                   -(HONO_emflux(jl,:)+NOx_emflux(jl,:))*delta_time

!			! MAQ_lg_20170524+
!			IF (jl == 2) THEN
!              print *,'messy_emdep_emis.f90; line 1706: HONO_NOX emissions: prescribing NO3s for Waldstein observations'
!              print *,'ENTER to continue'			  
!			  read (*,*)
!			ENDIF
!		    NO3s(jl)=1.2e19    ! MAQ_lg_20170524+ prescribed fixed NO3s based on Soergel et al. 2015 for Waldstein site
            ! MAQ_lg_20170524-
  ...
  END SUBROUTINE emdep_xtsurf_jNO3
  
  !=============================================================================
  ! Soil HONO emissions calculated from the HONO gradient and where the soil 
  ! HONO concentration is a function of pH, nitrite and soil moisture and temp.
  !==============================================================================

  SUBROUTINE emdep_xtsurf_soilHONO(istep, prhoa, pz, rahveg, ws, tsoil, HONO, HONO_slflux) 
  
    ! ----------------------------------------------------------------------------
    !   added an subroutine to estimate the soil HONO emission flux based on the
	!   studies by Su et al., 2011, Matthias Soergel et al.
    !
    !   Laurens Ganzeveld, May 2017
    ! ---------------------------------------------------------------------------
    ! Interface:
    ! ----------
    ! input 
    ...
    ! HONO      : HONO concentration understorey layer [mixing ratio]
    !
    ! output  
    ! HONO_slemflux  : soil HONO biogenic emission flux [molecules m-2 s-1]

    IMPLICIT NONE 

    ! I/O
    INTEGER,  INTENT(in)    :: istep
    REAL(dp), INTENT(in)    :: prhoa(:), pz, rahveg(:), ws(:), tsoil(:), HONO  
    REAL(dp), INTENT(out)   :: HONO_slflux(:)
	
    REAL, PARAMETER ::                                            &
        HONOsoil       =  7.5e-10, &  ! prescribed soil HONO mixing ratio of 15 ppbv to test the implementation 
		vt             =  1.e-3       ! exchange rate (s-1)

    !--- Local Variables:
    INTEGER   :: jl, klon
	REAL      :: recalc, dz, vtveg

    ! INITIALIZATION
    jl=istep ! SIZE(prc) ! ESS_lg_20120722+
    HONO_slflux(jl)=0._dp
    dz=pz
    vtveg=(1./(10.*rahveg(istep)))/dz ! calculation of rate constant of HONO flux out of soil	

    ! mz_LG_20020115 start of loop
    recalc=1.e3_dp*prhoa(jl)/(amd/avo)*dz          ! term to recalculate from mixing ratio to molecules m-2
    HONO_slflux(jl)=(HONOsoil-HONO)*vtveg*recalc   ! recalculation of flux from mixing ratios to molecules m-2 s-1

	HONO_slflux(jl)=(0.025+0.015*(2./10)*(tsoil(jl)-285.))*avo*1e-9  ! MAQ_lg_20170526+ prescribed flux based on Soergel et al. (2015)
	
    IF (istep==2) THEN
	   print *,'still implement properly the calculation of the exchange rate constant vtveg'
       print *,'HONO_slflux',HONO_slflux(jl), HONOsoil,HONO, dz, rahveg(istep),vtveg
	   read (*,*)
	ENDIF
	   
  END SUBROUTINE emdep_xtsurf_soilHONO

  ! MAQ_lg_20170517-

  ! MAQ_lg_20210727+

  SUBROUTINE emdep_xtsurf_Kh(istep, nveglay, ustar, hc, z, Kh_sl, Khout) 

    ! ----------------------------------------------------------------------------
    !   added a subroutine to calculate ncluded a subroutine of the Kh values following 
	!   the approach presented by Makar et al. 2017, with  Kh calculated based on the 
	!   near-field theory first proposed by Raupauch
    !
    !   Laurens Ganzeveld, July, 2021
    ! ---------------------------------------------------------------------------
    ! Interface:
    ! ----------
    ! input 
    ! nstep     : # of timesteps ! ESS_lg_20120722+
    ! nveglay   : number of canopy layers: multi-layer model
	! ustar     : friction velocity over vegetation
	! hc        : canopy height
	! z         : reference height layers
	! Kh_sl     : diffusvity of surface layer
    ! 
    ! output  
    ! Kh        : Eddy diffusivity for the exchange between the surface and canopy layers [m2 s-1]

    IMPLICIT NONE 

    ! I/O
 
    INTEGER,  INTENT(in)    :: istep, nveglay  ! ESS_lg_20120722+
    REAL(dp), INTENT(in)    :: ustar(:),hc(:),z(:),Kh_sl         
    REAL(dp), INTENT(out)   :: Khout(:,:)

    !--- Local Variables:
    INTEGER   :: jl, jk, klon
    REAL(dp)  :: taulagr(nveglay),sigmaw(nveglay),Kest(nveglay)
    ...
    !     canopy model needed for the subroutine xtsurf_veg_mlay
    DO jk=1,nveglay  
      ! MAQ_lg_20210727+ equation 4; calculation of lagrangian timescale
      taulagr(jk) = (hc(jl)/ustar(jl))*( (0.256*(z(jk)-0.75*hc(jl))/hc(jl)) +  &
     &       0.492*exp((-0.256*z(jk)/hc(jl))/0.492) )
      ! equation 5
      IF (z(jk) > hc(jl)) THEN
		sigmaw(jk)=1.25*ustar(jl)
      ELSE
		sigmaw(jk)=ustar(jl)*(0.75+0.5*cos(pi*(1.-z(jk)/hc(jl))))
      ENDIF
      Kest(jk)=(sigmaw(jk)**2)*taulagr(jk)          ! equation 3
      Khout(jl,jk)=(Kh_sl/Kest(1))*Kest(jk)     ! equation 2

	  !print *,'emdep_xtsurf_Kh: ',jl,jk,ustar(jl),z(jk),Kest(1),Kh_sl,Khout(jl,jk)
      !read (*,*)

    END DO

  END SUBROUTINE emdep_xtsurf_Kh
